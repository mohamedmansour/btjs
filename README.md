...
The remaining `f-` attributes will data bind that attribute by removing the prefix.

```html
<a f-href="url">website</a>
```

Whenever the signal `url` changes, it will update the `href` attribute for that element.
...
- `Attribute`: This type streams a dynamic signal to the attribute to the stream buffer.
