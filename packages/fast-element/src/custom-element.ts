...
const cssModuleMap = new Map<string, string | CSSStyleSheet>()
      ...
      internalStyleRender(shadowRoot: ShadowRoot) {
        if (definition.cssModule) {
          ...
          if (!cssModuleMap.has(definition.cssModule)) {
            ...
            const cssText = shadowRoot.querySelector('style')?.textContent
            if (cssText) {
            ...
            const cssCachedItem = cssModuleMap.get(definition.cssModule)! as string | CSSStyleSheet
